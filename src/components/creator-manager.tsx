...
export function CreatorManager({ creators, onCreatorsChange }: CreatorManagerProps) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t('manageCreators')}</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <Tabs defaultValue="single" className="w-full">
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="single">{t('addSingle')}</TabsTrigger>
            <TabsTrigger value="bulk">{t('bulkImport')}</TabsTrigger>
          </TabsList>
        ...
        <div className="space-y-2 max-h-60 overflow-y-auto">
          {creators.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-4">
              {t('noCreatorsAddedYet')}
            </p>
          ) : (
            <>
              <p className="text-sm text-muted-foreground">
                {t('totalCreators')} {creators.length}
              </p>
